...
/*
I included my questuions about this header, mainly focuses on the triangulation part.
First, did we implement two different ways to generate the internal triangulation, based on interior points p
and based on no-interior-points but a constraint edgeLength?
Second, if the my presume above is right, then there must be a little difference for these two triangulations while
we use them to generate the matrix A. Since in one kind, the interior points p are vertices of triangle mesh, while
in another kind, the triangle mesh are not rely on these p, so how can we compute the neighbour-distance of them? 
*/
...
//Here I have a question: what is the purpose of this template compute(const double edgeLength, std::vector<std::vector<double> > &bb)
//Since the Constructor is polygon vertices _v and the input points is p, so what does this edgeLength mean?
...
//These following three flags must be triangulation parameters in TriangulatorR2
...
//Still confused by the edgeLength param, because I can't understand how could edge length of average triangle affect.
//Update: I think the edgeLength is a strong constraint for the triangulation, in order to constrain
//        the generated triangle not to be too slim.
